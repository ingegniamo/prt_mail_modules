...
class PRTMailMessageDraft(models.Model):
    ...
    ref_partner_count = fields.Integer(
        string="Followers", compute="_compute__ref_partner_count"
    )
    ...
    # -- Count ref Partners
    def _compute__ref_partner_count(self):
        for rec in self:
        ...
        for rec in self:
            ...
            if rec.record_ref:
                ...
                if model_dict.get(rec.model, False)[1]:
                    subject_display = "{}: {}".format(
                        subject_display, rec.record_ref.name
                    )

                # Has subject
                if rec.subject:
                    subject_display = "{} => {}".format(subject_display, rec.subject)
    ...
    def unlink(self):
        if not self._name == "prt.mail.message.draft":
            self.env["prt.mail.message.draft"].sudo().search(
                [("model", "=", self._name), ("id", "in", self.ids)]
            ).sudo().unlink()
        return super().unlink()
...
class PRTMailComposer(models.TransientModel):
    ...
    current_draft_id = fields.Many2one(
        string="Draft", comodel_name="prt.mail.message.draft"
    )
