...
class TestMailMessageConversation(common.TransactionCase):
    ...
    def test_unlink_conversation_message(self):
        ...
        self.assertEqual(
            len(mail_message_1_ids), 0, msg="Mail Messages #1 count must be equal to 0"
        )
        mail_message_2_ids = self.MailMessage.search(
            [("id", "=", self.mail_message_2.id)]
        )
        self.assertEqual(
            len(mail_message_2_ids), 0, msg="Mail Messages #2 count must be equal to 0"
        )
    ...
    def test_unlink_all_conversation_message(self):
        ...
        self.assertEqual(
            len(empty_conversation_1), 0, msg="Conversations count must be equal to 0"
        )

    # TEST - 5 : Get or create new partner by Email
    def test_get_or_create_partner_id_by_email(self):
        """Get or create new partner by Email"""
        partner_id = self.CetmixConversation.get_or_create_partner_id_by_email(False)
        self.assertFalse(partner_id, msg="Partner must be False")
        ResPartner = self.env["res.partner"]
        email = "Test Partner <test.partner@example.com>"
        command = [("email", "=ilike", "partner.example@example.com")]
        partner_id = self.CetmixConversation.get_or_create_partner_id_by_email(email)
        self.assertEqual(
            self.res_partner_test.id,
            partner_id,
            msg="Partner id must be equal to {}".format(self.res_partner_test.id),
        )
        empty_partner = ResPartner.search(command, limit=1)
        self.assertEqual(empty_partner, ResPartner, msg="Recordset must be empty")
        created_partner_id = self.CetmixConversation.get_or_create_partner_id_by_email(
            "Partner Example <partner.example@example.com>"
        )
        partner_id = ResPartner.search(command, limit=1).id
        self.assertEqual(
            partner_id,
            created_partner_id,
            msg="Partner id must br equal to {}".format(created_partner_id),
        )
